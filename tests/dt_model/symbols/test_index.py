...
def test_index_unwraps_nested_index():
    ...
    Regression test for https://github.com/fbk-most/civic-digital-twins/issues/223:
    previously only TimeseriesIndex lacked this unwrap; Index has always had it,
    but this pins the behavior down explicitly.
    """
...
def test_index_rejects_mismatched_generic_index():
    """Index(name, a_timeseries_index) raises TypeError instead of silently misbehaving."""
    ts = TimeseriesIndex("ts")
    with pytest.raises(TypeError, match="TimeseriesIndex"):
        Index("bad", ts)  # type: ignore[arg-type]
...
def test_timeseries_index_unwraps_nested_timeseries_index():
    ...
    assert ts_copy.value is ts_base.node
