"""Tests for the civic_digital_twins.dt_model.engine.numpybackend.numpy_ast module."""
...
def test_round_trip():
    ...
    k = graph.constant(10)
    assert numpy_ast.graph_node_to_numpy_code(k) == f"n{k.id} = np.asarray(10)"

    p = graph.placeholder("p")
    pvalue = np.asarray([[10, 11, 12]])
    assert numpy_ast.graph_node_to_numpy_code(p, pvalue) == f"n{p.id} = np.asarray([[10, 11, 12]])"

    node = graph.add(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.add(n{k.id}, n{p.id})"

    node = graph.subtract(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.subtract(n{k.id}, n{p.id})"

    node = graph.multiply(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.multiply(n{k.id}, n{p.id})"

    node = graph.divide(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.divide(n{k.id}, n{p.id})"

    node = graph.equal(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.equal(n{k.id}, n{p.id})"

    node = graph.not_equal(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.not_equal(n{k.id}, n{p.id})"

    node = graph.less(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.less(n{k.id}, n{p.id})"

    node = graph.less_equal(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.less_equal(n{k.id}, n{p.id})"

    node = graph.greater(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.greater(n{k.id}, n{p.id})"

    node = graph.greater_equal(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.greater_equal(n{k.id}, n{p.id})"

    node = graph.logical_and(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.logical_and(n{k.id}, n{p.id})"

    node = graph.logical_or(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.logical_or(n{k.id}, n{p.id})"

    node = graph.logical_xor(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.logical_xor(n{k.id}, n{p.id})"

    node = graph.negate(k)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.negative(n{k.id})"

    node = graph.exp(k)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.exp(n{k.id})"

    node = graph.power(k, p)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.power(n{k.id}, n{p.id})"

    node = graph.log(k)
    assert numpy_ast.graph_node_to_numpy_code(node) == f"n{node.id} = np.log(n{k.id})"
...
def test_no_operation_for_node():
    ...
    with pytest.raises(numpy_ast.UnsupportedNodeType):
        numpy_ast.graph_node_to_ast_stmt(UnsupportedNode())
