...
class Model:
    def __init__(self):
        self.TS = TimeseriesIndex('time range',
                                  np.array([(t - pd.Timestamp('00:00:00')).total_seconds() for t in
        ...
        self.I_avg_cost = Index('average cost',
        ...
        self.indexes = [
            ...
            self.I_P_start_time, self.I_P_end_time, *self.I_P_cost, self.I_P_fraction_exempted,
            self.I_B_p50_cost,
            ...
            self.I_avg_cost, *self.I_fraction_rigid_euro, self.I_fraction_rigid, *self.I_modified_euro_class_split,
            self.I_delta_from_start, self.I_fraction_anticipating, self.I_number_anticipating,
            self.I_delta_to_end, self.I_fraction_postponing, self.I_number_postponing,
            self.I_total_anticipating, self.I_total_postponing, self.I_delta_before_start, self.I_number_anticipated,
            self.I_delta_after_end, self.I_number_postponed, self.I_number_shifted,
            self.I_modified_inflow,
            self.I_modified_starting, self.I_inflow_ratio, self.I_starting_ratio,
            self.I_total_base_inflow, self.I_total_modified_inflow,
            self.I_number_paying, self.I_total_paying, self.I_total_payed,
            self.I_total_anticipated, self.I_total_postponed, self.I_total_shifted,
            self.I_traffic, self.I_modified_traffic,
            self.I_traffic_ratio,
            self.I_average_emissions, self.I_modified_average_emissions,
            self.I_emissions, self.I_modified_emissions,
    ...
    def evaluate(self, size=1):
        ...
        for index in self.indexes:
            if hasattr(index.value, 'rvs'):
                initial_values[index.node] = np.expand_dims(
                    np.array(index.value.rvs(size=size)), axis=1
                )
...
def compute_kpis(m, evals):
    return {
        ...
        'Modified emissions [NOx gr/day]': int(evals[m.I_total_emissions].mean()) - int(
            evals[m.I_total_modified_emissions].mean())
    }
