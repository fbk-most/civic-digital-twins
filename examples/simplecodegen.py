"""Simple interim example to test the codegen features.
...
from civic_digital_twins.dt_model.engine.numpybackend import bytecode
...
def main():
    ...
    # compile each tree to bytecode
    ccache = bytecode.ThreadSafeCodeCache()
    for tree in trees:
        ccache.compile(tree)

    # print the functions that we have created
    print("=== functions ===")
    for fx in ccache.all():
        print(fx.fdef.unparse())
        print("")
    print("")

    # print the bytecode that we have generated
    print("=== bytecode ===")
    for fx in ccache.all():
        print(fx.disassemble())
        print("")
    print("")
    ...
    for tree in trees:
        bytecode.evaluate_tree(state, tree)
